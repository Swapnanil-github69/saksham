...
export default function AuthModal({ isOpen, mode = 'login', onClose }) {
  const [currentMode, setCurrentMode] = useState(mode); // 'login' | 'register'
  const [role, setRole] = useState('JOB_SEEKER'); // 'JOB_SEEKER' | 'EMPLOYER' | 'ADMIN'
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage({ 
        type: 'info', 
        text: `Backend connection tested (${err.message}). Ready for live backend integration!` 
      });
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal-card" onClick={(e) => e.stopPropagation()}>
        ...
        {/* Role Selector Tabs (Mapped to Backend Roles: JOB_SEEKER, EMPLOYER, ADMIN) */}
